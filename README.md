...
- **Mining** — Autolykos v2 candidate assembly with EIP-27 re-emission, solution validation. Mempool transactions are selected under both protocol limits, fees collected into a reward box capped by measured serialized size, and the candidate is rebuilt on TTL expiry so work stays available between blocks
