...
pub(super) struct PagedCachedFile {
    file: CheckedBackend,
    page_size: u64,
    max_read_cache_bytes: AtomicUsize,
...
impl PagedCachedFile {
    pub(super) fn new(
        ...
        Ok(Self {
            ...
            page_size,
            max_read_cache_bytes: AtomicUsize::new(max_read_cache_bytes),
    ...
    fn flush_write_buffer(&self) -> Result {
        ...
        for (offset, buffer) in write_buffer.cache.iter_mut() {
            ...
            if cache_size + buffer.len() <= self.max_read_cache_bytes.load(Ordering::Relaxed) {
    ...
    pub(super) fn read(&self, offset: u64, len: usize, hint: PageHint) -> Result<Arc<[u8]>> {
        ...
        let mut removed = 0;
        if cache_size + len > self.max_read_cache_bytes.load(Ordering::Relaxed) {
            while removed < len {
    ...
    pub(super) fn set_max_read_cache_bytes(&self, bytes: usize) {
        self.max_read_cache_bytes.store(bytes, Ordering::Relaxed);
        // If the new limit is smaller, evict excess entries.
        let current = self.read_cache_bytes.load(Ordering::Relaxed);
        if current > bytes {
            for cache_slot in 0..self.read_cache.len() {
                let mut lock = self.read_cache[cache_slot].write().unwrap();
                while self.read_cache_bytes.load(Ordering::Relaxed) > bytes {
                    if let Some((_, removed)) = lock.pop_lowest_priority() {
                        self.read_cache_bytes
                            .fetch_sub(removed.len(), Ordering::AcqRel);
                    } else {
                        break;
