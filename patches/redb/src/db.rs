...
impl Database {
    ...
    /// Clear the read cache, immediately freeing all cached pages.
    ///
    /// Subsequent reads will repopulate the cache from disk up to the
    /// current cache limit. This is useful for reducing memory usage
    /// after bulk operations (e.g. initial sync → tip transition).
    pub fn clear_read_cache(&self) {
        self.mem.clear_read_cache();
    }

    /// Set the read cache size limit at runtime.
    ///
    /// If the new limit is smaller than the current cache usage, excess
    /// entries are evicted immediately. Call [`clear_read_cache`](Self::clear_read_cache)
    /// afterwards to free all cached pages and start fresh under the new limit.
    pub fn set_read_cache_limit(&self, bytes: usize) {
        self.mem.set_read_cache_limit(bytes);
    }
