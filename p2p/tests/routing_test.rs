...
fn router_modifier_request_no_inv_no_outbound_drops() {
    ...
    assert!(
        actions.is_empty(),
        "no outbound peers means no fallback target"
    );
}
...
fn router_modifier_request_with_inv_ignores_fallback() {
    ...
    assert_eq!(
        targets,
        vec![PeerId(2)],
        "inv table target should be used, not fallback"
    );
}
...
fn router_fallback_request_response_reaches_requester() {
    ...
    assert_eq!(
        targets,
        vec![PeerId(2)],
        "response should route back to original requester"
    );
}
...
fn router_consumed_code_suppresses_forwarding() {
    ...
    assert!(
        actions.is_empty(),
        "consumed code should not produce Send actions"
    );
}
...
fn router_multiple_consumed_codes() {
    ...
    for code in [76, 78, 80, 90, 91] {
        ...
        assert!(
            actions.is_empty(),
            "consumed code {} should be suppressed",
            code
        );
    }
...
fn request_tracker_cap_eviction_and_sweep() {
    ...
    assert_eq!(
        tracker.lookup(&overflow_id),
        Some(PeerId(2)),
        "new entry must survive eviction"
    );
...
fn request_tracker_duplicate_id_overwrites() {
    ...
    assert_eq!(
        tracker.len(),
        1,
        "duplicate insert should not grow pending count"
    );
    ...
    assert_eq!(
        result,
        Some(PeerId(2)),
        "fulfill should return the latest peer"
    );
    ...
    assert_eq!(
        tracker.fulfill(&id),
        None,
        "entry should be gone after single fulfill"
    );
}
...
fn request_tracker_fulfill_after_sweep() {
    ...
    assert_eq!(
        tracker.fulfill(&[0xaa; 32]),
        None,
        "fulfill after sweep should return None"
    );
}
...
fn router_consumed_code_boundary_values() {
    ...
    assert_eq!(
        targets,
        vec![PeerId(2)],
        "non-consumed code 1 should forward"
    );
}
...
fn router_consumed_code_double_register() {
    ...
    assert!(
        actions.is_empty(),
        "double-registered code should still suppress"
    );
    ...
    assert!(
        !actions.is_empty(),
        "non-consumed code should still forward after double-register"
    );
}
...
fn router_consumed_code_from_outbound_suppresses() {
    ...
    assert!(
        actions.is_empty(),
        "consumed code from outbound should suppress, not forward to inbound"
    );
}
...
fn router_non_consumed_unknown_unaffected_by_registration() {
    ...
    let mut targets: Vec<PeerId> = actions
        .iter()
        .filter_map(|a| match a {
            Action::Send { target, .. } => Some(*target),
            _ => None,
    ...
    assert_eq!(
        targets,
        vec![PeerId(2), PeerId(3)],
        "non-consumed code should forward to opposite direction"
    );
    ...
    assert_eq!(
        targets,
        vec![PeerId(1)],
        "non-consumed code from inbound should forward to outbound"
    );
}
...
fn request_tracker_interleaved_record_and_sweep() {
    ...
    assert_eq!(
        tracker.lookup(&[0xaa; 32]),
        None,
        "entry A should be expired"
    );
    assert_eq!(
        tracker.lookup(&[0xbb; 32]),
        Some(PeerId(2)),
        "entry B should survive"
    );
    ...
    assert_eq!(
        tracker.lookup(&[0xbb; 32]),
        None,
        "entry B should be expired with TTL=0"
    );
    assert_eq!(
        tracker.len(),
        0,
        "tracker should be empty after TTL=0 sweep"
    );
}
